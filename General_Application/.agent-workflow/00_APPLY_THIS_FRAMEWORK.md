...
3) Evidence Gate  
No claims of completion without commands/logs recorded in `.agent-workflow/Progress.txt` (or per-task logs merged into it).
...
- Sisyphus merges essential evidence into `.agent-workflow/Progress.txt`.
...
- `Progress.txt` : append-only evidence log (merged from per-task logs)
